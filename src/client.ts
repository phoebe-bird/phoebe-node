...
export interface ClientOptions {
   ...
   * Set the log level.
   *
   * Defaults to process.env['PHOEBE_LOG'] or 'warn' if it isn't set.
   */
...
export class Phoebe {
   ...
   */
  constructor({
    baseURL = readEnv('PHOEBE_BASE_URL'),
    apiKey = readEnv('EBIRD_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
    ...
    this.timeout = options.timeout ?? Phoebe.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('PHOEBE_LOG'), "process.env['PHOEBE_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('PHOEBE_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.ebird.org/v2';
  }
  ...
  protected async authHeaders(opts: FinalRequestOptions): Promise<NullableHeaders | undefined> {
    return buildHeaders([{ 'X-eBirdApiToken': this.apiKey }]);
  }
