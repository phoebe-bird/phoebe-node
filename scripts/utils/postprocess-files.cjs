...
const pkgImportPath = process.env['PKG_IMPORT_PATH'] ?? '@stainless-api/github-internal/';
...
async function postprocess() {
  for await (const file of walk(path.resolve(__dirname, '..', '..', 'dist'))) {
    ...
    if (file.endsWith('.d.ts')) {
      ...
      // if we have `import { type Readable } from '@stainless-api/github-internal/_shims/index'`,
